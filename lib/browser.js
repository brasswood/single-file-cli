...
const { build, makeTempDir, Command, errors, mkdir, writeTextFile, remove } = Deno;
...
async function launchBrowser(options = {}, indexPath = 0) {
	...
	if (options.crawlDownloads) {
		args = args.filter(arg => arg !== "--no-startup-window");
	}
...
async function initializeProfile(profilePath) {
	...
	await writeTextFile(preferencesPath, JSON.stringify({
		plugins: {
...
async function closeBrowser() {
	if (child !== undefined) {
		...
		try {
			...
			// The browser may already have exited with a non-zero status.
		}
		child = undefined;
	}
