...
async function setupNetworkInterception({ Browser, Emulation, Fetch, Network }, pageContext) {
	...
	await setupDownloadBehavior({ Browser }, pageContext);
}
...
async function setupDownloadBehavior({ Browser }, { options, debugMessages, downloadState }) {
	...
	Browser.addEventListener(DOWNLOAD_PROGRESS_EVENT_TYPE, ({ params }) => {
		...
		if (params.state === COMPLETED_DOWNLOAD_STATE) {
			if (params.filePath) {
				download.filename = getFilenameFromPath(params.filePath);
			}
			logData(["Completed download", download.url, download.filename], { options, debugMessages });
			download.resolve({
				download: true,
	...
	await Browser.setDownloadBehavior({ behavior: ALLOW_BEHAVIOR, downloadPath: options.outputDirectory, eventsEnabled: EVENTS_ENABLED });
}
	...
	try {
		...
		if (navigateResult && navigateResult.isDownload && options.crawlDownloads) {
			...
			return await Promise.race([
				waitForDownload(downloadState, navigateResult.frameId),
				waitForTimeout(loadTimeoutAbortSignal, options.browserLoadMaxTime, LOAD_TIMEOUT_ERROR_MESSAGE, LOAD_TIMEOUT_ERROR)
			]);
		}
