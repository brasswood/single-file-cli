...
let browserClient, downloadState;
...
async function setupNetworkInterception({ Emulation, Fetch, Network }, pageContext) {
...
async function setupDownloadBehavior({ Browser }, { options, debugMessages, downloadState }) {
	const DENY_BEHAVIOR = "deny";
	const ALLOW_BEHAVIOR = "allow";
	const DOWNLOAD_WILL_BEGIN_EVENT_TYPE = "downloadWillBegin";
	const DOWNLOAD_PROGRESS_EVENT_TYPE = "downloadProgress";
	...
	Browser.addEventListener(DOWNLOAD_WILL_BEGIN_EVENT_TYPE, ({ params }) => {
	...
	Browser.addEventListener(DOWNLOAD_PROGRESS_EVENT_TYPE, async ({ params }) => {
...
async function getContextId({ Debugger, Page, Runtime }, { options, debugMessages, downloadState }) {
	const [contextId] = await Promise.all([
		loadPage({ Page, Runtime }, { options, debugMessages, downloadState }),
...
async function loadPage({ Page, Runtime }, { options, debugMessages, downloadState }) {
...
async function getTopFrameContextId({ Page, Runtime }, { options, debugMessages }) {
	...
	const state = { topFrameId: undefined, contexts: new Map() };
	...
	try {
		...
		await assertTopFrameId({ Page }, state.topFrameId);
		const contextId = await findValidSingleFileContext({ Runtime }, state.contexts, state.topFrameId, { options, debugMessages });
		return contextId;
	} finally {
...
function setupContextCreatedListener({ Runtime }, state) {
	...
	const onContextCreated = ({ params }) => {
		...
		if (context.name === SINGLE_FILE_WORLD_NAME) {
...
async function assertTopFrameId({ Page }, topFrameId) {
	const { frameTree } = await Page.getFrameTree();
	const currentTopFrameId = frameTree.frame.id;
	if (topFrameId !== currentTopFrameId) {
		throw new Error(`Top frame ID mismatch: frameNavigated reported ${topFrameId}, but Page.getFrameTree reported ${currentTopFrameId}`);
	}
}
...
async function findValidSingleFileContext({ Runtime }, contexts, topFrameId, { options, debugMessages }) {
	...
	while (Date.now() - startTime < CONTEXT_RETRY_TIMEOUT) {
		const contextIds = Array.from(contexts)
			.filter(([, frameId]) => frameId === topFrameId)
